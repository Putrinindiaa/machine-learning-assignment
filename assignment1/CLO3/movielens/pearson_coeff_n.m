...
function [ output ] = pearson_coeff_n( idMovie1 , n)
% fungsi jaccard_coeff_n digunakan untuk mencari film-film dengan Pearson Correlation Coeffision terbesar sebanyak n.
...
%     diskip, karena itu sama saja dengan membandingkan 'Pearson Correlation Coeffision' pada titik yang sama.
    if i == idMovie1
        continue;
    end
%     Menghitung 'Jaccard Coeffcient' film inputan dengan film id ke i,
%     lalu memasukkannya hasilnya ke dalam mat_coeffn(1).
    mat_coeffn(i,1) = pearson_corrcoeff(idMovie1,itemids(i));
    if  isnan(pearson_corrcoeff(idMovie1,itemids(i))) == 1
        mat_coeffn(i,1) = 0;
    else 
        mat_coeffn(i,1) = pearson_corrcoeff(idMovie1,itemids(i));
    end
    mat_coeffn(i,2) = i;
%     Menampilkan detail iterasi, idMovie, jumlah PCC nya,
%     dan judul film.
...
% Menampilkan film-film dengan Pearson Corrcoeff terbesar sebanyak n
display_f = sprintf('Showing movies that related to %s', items{idMovie1});
disp(display_f);
for i=1:n
    display_final = sprintf('ID : %d \t | %s \t | PCC : %.3f', X(i,2), items{X(i,2)}, X(i,1));
    disp(display_final);
end
